...
function Get-SwaggerSpecPathInfo
{
    ...
    $JsonPathItemObject.value.PSObject.Properties | ForEach-Object {
        ...
        $paramObject = Convert-ParamTable -ParamTable $paramInfo -ParameterSetName $operationId
...
function New-SwaggerPath
{
    ...
    foreach ($parameterSetDetail in $parameterSetDetails) {
        # TODO: This description ends up looking weird, especially when one or more end in punctuation, or one or more have more than one sentence in them
        if ($description) {
            $description += ", or $($parameterSetDetail.Description)"
        } else {
